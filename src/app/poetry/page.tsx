...
const poetVerses: PoetVerse[] = [
  { poet: 'مرزا غالب', image: 'https://upload.wikimedia.org/wikipedia/commons/d/de/Mirza_Ghalib_portrait.jpg', first: 'ہزاروں خواہشیں ایسی کہ ہر خواہش پہ دم نکلے', second: 'بہت نکلے مرے ارمان لیکن پھر بھی کم نکلے', genre: 'غزل' },
  { poet: 'علامہ اقبال', image: 'https://upload.wikimedia.org/wikipedia/commons/e/ef/Allama_Iqbal_1934.jpg', first: 'خودی کو کر بلند اتنا کہ ہر تقدیر سے پہلے', second: 'خدا بندے سے خود پوچھے بتا تیری رضا کیا ہے', genre: 'نظم' },
  { poet: 'فیض احمد فیض', image: 'https://upload.wikimedia.org/wikipedia/commons/4/4b/Faiz_Ahmed_Faiz_1969.jpg', first: 'مجھ سے پہلی سی محبت مرے محبوب نہ مانگ', second: 'میں نے سمجھا تھا کہ تو ہے تو درخشاں ہے حیات', genre: 'غزل' },
  { poet: 'جان ایلیا', image: 'https://upload.wikimedia.org/wikipedia/commons/b/ba/Jaun_Elia.jpg', first: 'شرم، وحشت، جھجک، پریشانی', second: 'ناز سے کام کیوں نہیں لیتیں', genre: 'غزل' },
  { poet: 'میر تقی میر', image: 'https://upload.wikimedia.org/wikipedia/commons/8/89/Portrait_Placeholder.png', first: 'پتہ پتہ بوٹا بوٹا حال ہمارا جانے ہے', second: 'جانے نہ جانے گل ہی نہ جانے باغ تو سارا جانے ہے', genre: 'غزل' },
  { poet: 'احمد فراز', image: 'https://upload.wikimedia.org/wikipedia/commons/8/89/Portrait_Placeholder.png', first: 'اب کے ہم بچھڑے تو شاید کبھی خوابوں میں ملیں', second: 'جس طرح سوکھے ہوئے پھول کتابوں میں ملیں', genre: 'غزل' },
];
...
function PoetVerseCard({ item }: { item: PoetVerse }) {
  ...
  const handleWhatsApp = () => {
    const text = encodeURIComponent(`"${item.first}\n${item.second}"\n\n— ${item.poet}\n— Shared via Azad Khabar`);
    window.open(`https://wa.me/?text=${text}`, '_blank');
  };

  return (
    <article className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-5 flex items-center gap-4 hover:shadow-md transition-shadow">
      <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-gray-200 dark:border-gray-600 shadow-sm flex-shrink-0">
        {!imgError ? (
          <img
            src={item.image}
            alt={item.poet}
          ...
          <div className="w-full h-full flex items-center justify-center text-white text-sm font-bold bg-gradient-to-br from-primary-500 to-primary-700">
            {item.poet.charAt(0)}
          </div>
        )}
      </div>
      <div className="flex-1 min-w-0 font-nastaliq">
        <span className="inline-block bg-primary-50 dark:bg-primary-900/30 text-primary-600 dark:text-primary-400 text-xs font-semibold px-2.5 py-0.5 rounded-full mb-2">
          {item.genre}
        </span>
        <div className="leading-[2.2] text-sm md:text-base text-gray-900 dark:text-gray-100">
          <p className="truncate">{item.first}</p>
          <p className="truncate">{item.second}</p>
        </div>
        <div className="flex items-center justify-between mt-2">
          <p className="text-xs text-gray-500 dark:text-gray-400">{item.poet}</p>
          <div className="flex items-center gap-1.5">
            <button onClick={handleCopy} className="text-gray-400 hover:text-primary-600 dark:hover:text-primary-400 transition-colors p-1" title="کاپی کریں">
  ...
  return (
    <>
      <div className="mx-auto px-4 py-6" style={{ maxWidth: '1200px' }}>
        ...
        {/* ──────── Master Poets — Combined Verse + Portrait ──────── */}
        <section className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {poetVerses.map((item, i) => (
              <PoetVerseCard key={i} item={item} />
            ))}
          </div>
        </section>
