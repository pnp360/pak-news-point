...
export async function generateMetadata({ params }: Props) {
  ...
  if (!author) return { title: 'مصنف نہیں ملا' };
  return { title: `${author.name || 'مصنف'} - خبریں`, description: `${author.name} کی تحریر کردہ خبریں` };
}
  ...
  return (
    <div className="container mx-auto px-4 py-6">
      <div className="bg-white rounded-xl shadow-sm p-6 mb-6 flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold">{author.name || 'مصنف'}</h1>
