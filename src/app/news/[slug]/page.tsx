    ...
    <div className="container mx-auto px-4 py-6 max-w-7xl">
      ...
      <div className="flex flex-col lg:flex-row gap-10">
        <article className="flex-1 min-w-0">
          ...
          {/* Meta Bar */}
          <div className="flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-gray-500 mb-6 pb-5 border-b border-gray-200">
            {article.author.name && (
          ...
          {article.tags.length > 0 && (
            <div className="flex flex-wrap items-center gap-2 mt-10 pt-6 border-t border-gray-200">
              <span className="text-sm font-bold text-gray-600 ml-2">ٹیگز:</span>
              {article.tags.map((at) => (
                <Link
                  key={at.tag.id}
                  href={`/tag/${at.tag.slug}`}
                  className="bg-gray-50 hover:bg-primary-50 hover:text-primary-600 border border-gray-200 px-3 py-1 rounded-full text-sm transition-colors"
                >
                  {at.tag.name}
                </Link>
              ))}
          ...
          <div className="mt-8 pt-6 border-t border-gray-200">
            <div className="flex items-center gap-4">
              <span className="text-sm font-bold text-gray-600">اشتراک کریں:</span>
